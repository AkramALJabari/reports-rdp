...
if(!msg.member.roles.cache.some(role => role.id === `961922673888329738`))return msg.channel.send({ embeds: [new MessageEmbed()
...
.setAuthor({ name:msg.author.username, iconURL:msg.author.avatarURL() })
.setDescription(`**${data.prefix}block-gangs [id user]**`)
.setColor(`#2F3136`)] })
let id = args[0];
...
.setDescription(`**لا يمكنني العثور على هذا العضو**`)
.setColor(`#2F3136`)] });
fs.readFile('./blocks-gangs.json', 'utf8', (err, datadb) => {
...
fs.writeFileSync('./blocks-gangs.json', new_json, 'utf8');
